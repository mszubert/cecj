...
public abstract class RandomizedPlayerFitnessCalculator extends
		GamePlayerFitnessCalculator {

	@Override
	protected GameScenario getInverseScenario(EvolutionState state,
			Player player) {
		return new GameScenario(new Player[] { getOpponent(), player },
				new double[] { evaluatorRandomness, evaluatedRandomness });
	}

	@Override
	protected GameScenario getScenario(EvolutionState state, Player player) {
		return new GameScenario(new Player[] { player, getOpponent() },
